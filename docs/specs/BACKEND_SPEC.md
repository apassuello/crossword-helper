...
**Frontend Usage:** `PatternMatcher.jsx` - Real-time progress for large searches
...
**Behavior:**
- Places black squares at both positions
- Updates cell properties (`isBlack: true`, clears letter)
- Removes theme locks from affected cells
...
**Purpose:** Cancel a running autofill task

**Status:** ⚠️ **NOT YET IMPLEMENTED** - Documented for Phase 3.1 implementation

**Request:**
```bash
curl -X POST http://localhost:5000/api/fill/cancel/task_abc123
```
...
```json
{
  ...
  "state_saved": true,
  "state_file": "/tmp/autofill_state_task_abc123.json"
}
```

**Behavior:**
1. Set pause flag via `PauseController` (triggers graceful stop)
2. Save current CSP state for potential resume
3. Clean up background task resources
4. Return confirmation

**vs. Pause:**
- **Cancel**: Stops task, optionally saves state, user expects task to end
- **Pause**: Saves state, user explicitly plans to resume later

**Status Codes:**
- `200 OK` - Cancel request processed
- `404 Not Found` - Task ID not found or already completed
- `500 Internal Server Error` - Cancel failed

**Frontend Usage:** `App.jsx` - handleCancelAutofill calls this endpoint

**Implementation Notes** (for Phase 3.1):
- Use existing `PauseController.request_pause()` infrastructure
- Similar to pause, but marks state as "cancelled" vs "paused"
- Frontend should call this before closing SSE connection
- State cleanup can happen via DELETE /api/fill/state/{task_id} after cancel
...
All endpoints use standard HTTP status codes consistently:

#### Success Codes (2xx)

| Status Code | Usage | Typical Endpoints |
|-------------|-------|-------------------|
| `200 OK` | Successful operation, returning data | GET requests, most POST operations |
| `201 Created` | Resource successfully created | POST /api/wordlists/:name |
| `202 Accepted` | Async task started, processing in background | POST /api/fill/with-progress, POST /api/pattern/with-progress |

#### Client Error Codes (4xx)

| Status Code | Usage | Common Scenarios |
|-------------|-------|------------------|
| `400 Bad Request` | Invalid input, missing required fields | Missing grid, invalid pattern, malformed JSON |
| `403 Forbidden` | Action not permitted | Deleting core wordlists, modifying protected resources |
| `404 Not Found` | Resource doesn't exist | Task ID not found, wordlist doesn't exist, endpoint not found |
| `409 Conflict` | Business logic conflict | Theme placement conflicts, edits create unsolvable state, wordlist name already exists |

#### Server Error Codes (5xx)

| Status Code | Usage | Common Scenarios |
|-------------|-------|------------------|
| `500 Internal Server Error` | Unexpected server error | Validation failed, processing error, unexpected exception |
| `503 Service Unavailable` | Service health issue | CLI adapter unhealthy |
| `504 Gateway Timeout` | Grid numbering timeout | CLI subprocess exceeded timeout (operation-specific) |
| `505 HTTP Version Not Supported` | Pattern search timeout | CLI subprocess exceeded timeout (operation-specific) |
| `506 Variant Also Negotiates` | Normalization timeout | CLI subprocess exceeded timeout (operation-specific) |
| `507 Insufficient Storage` | Grid fill timeout | CLI subprocess exceeded timeout (operation-specific) |

**Note on Timeout Codes**: Status codes 504-507 are repurposed for operation-specific timeouts to allow clients to distinguish which operation timed out. Standard HTTP semantics are maintained (5xx = server error).
...
```json
{
  "error": "Task ID abc123 not found"
}
```
...
```json
{
  "error": "Theme placement conflicts with existing letter at (0,5): expected T, found A"
}
```
...
```json
{
  "error": "Failed to validate grid structure"
}
```

### Error Code Strings (Legacy)

Some endpoints may return structured error codes for programmatic handling:
...
**Note**: Most endpoints use simple string error messages. Structured error codes are being phased out in favor of HTTP status codes + descriptive messages.
