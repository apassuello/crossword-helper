...
class BeamSearchState:
    ...
    beam: List[
        Dict
    ]  # Each dict contains: grid_dict, used_words, slot_assignments, etc.
...
class StateManager:
    ...
    def capture_csp_state(
        ...
        domains_list = {
            slot_id: list(words) for slot_id, words in autofill_instance.domains.items()
        }
        ...
        return CSPState(
            ...
            letter_frequency_table=getattr(
                autofill_instance, "letter_frequency_table", None
            ),
        )
    ...
    def restore_to_autofill(autofill_instance, csp_state: CSPState) -> None:
        ...
        autofill_instance.domains = {
            slot_id: set(words) for slot_id, words in csp_state.domains.items()
        }
    ...
    def serialize_beam_state(beam_state_obj) -> Dict:
        ...
        return {
            ...
            "domains": {
                json.dumps(list(k)): v for k, v in beam_state_obj.domains.items()
            },
            "domain_reductions": {
                json.dumps(list(k)): v
    ...
    def capture_beam_search_state(
        orchestrator_instance, beam: List, filled_slots: Set, slot_idx: int
    ) -> BeamSearchState:
        """
        ...
        recently_failed = [
            list(slot_tuple) for slot_tuple in orchestrator_instance.recently_failed
        ]
