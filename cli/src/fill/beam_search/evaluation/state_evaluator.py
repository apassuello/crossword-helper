...
class StateEvaluationStrategy(ABC):
    ...
    @abstractmethod
    def evaluate_state_viability(self, state: BeamState, last_filled_slot: Optional[Dict] = None) -> Tuple[bool, float]:
        """
...
class StateEvaluator(StateEvaluationStrategy):
    ...
    def __init__(self, pattern_matcher, get_min_score_func, get_intersecting_slots_func):
        """
    ...
    def evaluate_state_viability(self, state: BeamState, last_filled_slot: Optional[Dict] = None) -> Tuple[bool, float]:
        ...
        Goes beyond binary viability (possible/impossible) to assess risk level.
        ...
        for slot in slots_to_check:
            ...
            available_candidates = [(word, score) for word, score in candidates if word not in state.used_words]
