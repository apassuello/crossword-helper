...
class BeamSearchOrchestrator:
    ...
    def _init_components(self):
        ...
        threshold_ordering = ThresholdDiverseOrdering(threshold=50, temperature=0.8)
        stratified_ordering = StratifiedValueOrdering(tier_size=5)
        # Composite value ordering (theme priority FIRST, then LCV + threshold-diverse + stratified)
        ...
        while len(filled_slots) < total_slots:
            ...
            if self.pause_controller and self.iterations % 10 == 0:
                if self.pause_controller.should_pause():
                    ...
                    self._save_state_and_pause(beam, filled_slots, slot_idx, all_slots, total_slots)
            ...
            unfilled_slots = [s for s in all_slots if (s["row"], s["col"], s["direction"]) not in filled_slots]
            ...
            if not expanded_beam:
                ...
                if attempts < self.max_attempts_per_slot:
                    penalty = 1000 * len(self.recently_failed)
                    logger.debug(f"  Removing slot from filled_slots, MRV will deprioritize it " f"(penalty={penalty})")
            ...
            # Adaptive beam width
            adaptive_width = self.beam_manager.get_adaptive_beam_width(beam, unfilled_slots, total_slots, slot)
    ...
    def _prepare_initial_state(self, all_slots: List[Dict]) -> Tuple:
        ...
        if self.theme_entries:
            sorted_slots, theme_words = self.slot_selector.prioritize_theme_entries(all_slots)
        else:
    ...
    def _analyze_slot_conflicts(self, state: BeamState, slot: Dict) -> Set[Tuple[int, int, str]]:
        ...
        for slot_id, word in state.slot_assignments.items():
            ...
            if len(candidates_without) > 0 and len(candidates_with) == 0:
                ...
                logger.debug(
                    f"    Conflict detected: slot {slot_id} with letter "
                    f"'{constraint_letter}' at pos {intersection_pos_current}"
                )
    ...
    def _backtrack_beam_states(self, beam: List[BeamState], depth: int = 1) -> List[BeamState]:
        ...
        This is REAL backtracking - undoes previous assignments to explore
        alternative paths, unlike fake backtracking that just tries more
        candidates for the same stuck slot.
    ...
    def _resume_fill(self, resume_state, timeout: int):
        ...
        beam = [StateManager.deserialize_beam_state(beam_dict) for beam_dict in resume_state.beam]
            ...
            for slot in all_slots:
                ...
                if slot_id not in best_state.slot_assignments:
                    ...
                    if self.state_evaluator.is_gibberish_pattern(pattern, strict=self.partial_fill_mode):
                        best_state.grid.remove_word(slot["row"], slot["col"], slot["length"], slot["direction"])
                    problematic_slots.append(slot)

        # Calculate actual filled slots
        actual_filled = sum(1 for slot in all_slots if "?" not in best_state.grid.get_pattern_for_slot(slot))
