        ...
        for i, letter in enumerate(word):
            if direction == "across":
                ...
                if target_col >= len(grid[row]):
                    conflicts.append(
                        f"Position ({target_row}, {target_col}) out of bounds"
                    )
                    continue
            else:  # down
                target_row = row + i
            ...
            if is_black:
                conflicts.append(
                    f"Cannot place '{letter}' at ({target_row}, {target_col}): cell is black"
                )
        ...
        if direction == "across":
            ...
            if col > 0:
                ...
                if isinstance(prev_cell, str) and (prev_cell == "." or prev_cell == ""):
                    grid[row][col - 1] = "#"
                elif isinstance(prev_cell, dict) and not prev_cell.get(
                    "isBlack", False
                ):
            ...
            if end_col < len(grid[row]):
                ...
                if isinstance(next_cell, str) and (next_cell == "." or next_cell == ""):
                    grid[row][end_col] = "#"
                elif isinstance(next_cell, dict) and not next_cell.get(
                    "isBlack", False
                ):
