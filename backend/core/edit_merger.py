...
class EditMerger:
    ...
    def merge_edits(
        ...
        changes = self._detect_changes(
            saved_grid, edited_grid, saved_state.slot_list, saved_state.slot_id_map
        )
        ...
        if has_domains:
            validation_result = self._validate_state(
                updated_domains, saved_state.constraints, saved_state.slot_list
            )
    ...
    def _detect_changes(
        self, saved_grid, edited_grid, slot_list: List[Dict], slot_id_map: Dict
    ) -> GridChanges:
        """
        ...
        for slot in slot_list:
            ...
            if saved_has_gaps and not edited_has_gaps:
                ...
                not saved_has_gaps
                and not edited_has_gaps
                and saved_pattern != edited_pattern
            ):
    ...
    def _update_domains_with_edits(
        ...
        updated_domains = {
            int(slot_id): set(words) for slot_id, words in saved_state.domains.items()
        }
