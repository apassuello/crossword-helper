...
def test_cli_can_parse_11x11_grid(tmp_path):
    ...
    try:
        ...
        assert "AttributeError" not in result.stderr, f"CLI crashed parsing 11x11 grid: {result.stderr}"

        # Should load the grid successfully
        assert (
            "Loading grid" in result.stderr or result.returncode == 0 or "timeout" in result.stderr.lower()
...
def test_cli_can_parse_15x15_grid(tmp_path):
    ...
    try:
        ...
        assert "AttributeError" not in result.stderr, f"CLI crashed parsing 15x15 grid: {result.stderr}"
...
def test_cli_can_parse_21x21_grid(tmp_path):
    ...
    try:
        ...
        assert "AttributeError" not in result.stderr, f"CLI crashed parsing 21x21 grid: {result.stderr}"
...
def test_bug_regression_frontend_dict_causes_crash(tmp_path):
    ...
    assert "'dict' object has no attribute" in result.stderr, "Expected specific error message about dict not having attribute"
...
def test_validate_grid_dimensions():
    ...
    for grid_data, expected_size in grids:
        assert len(grid_data["grid"]) == expected_size, f"Grid has {len(grid_data['grid'])} rows, expected {expected_size}"

        for i, row in enumerate(grid_data["grid"]):
            assert len(row) == expected_size, f"Row {i} has {len(row)} cells, expected {expected_size}"
