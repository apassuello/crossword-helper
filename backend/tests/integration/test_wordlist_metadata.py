...
class TestWordlistMetadataStructure:
    ...
    def test_all_wordlists_have_name_field(self):
        ...
        assert len(missing_name) == 0, f"These wordlists are missing 'name' field: {missing_name}"
    ...
    def test_all_wordlists_have_required_fields(self):
        ...
        for key, info in metadata["wordlists"].items():
            for field in required_fields:
                assert field in info, f"Wordlist '{key}' missing required field '{field}'"
    ...
    def test_custom_wordlists_have_name_field(self):
        ...
        custom_wordlists = {key: info for key, info in metadata["wordlists"].items() if info.get("category") == "custom"}

        assert len(custom_wordlists) > 0, "No custom wordlists found for testing"

        for key, info in custom_wordlists.items():
            assert "name" in info, f"Custom wordlist '{key}' missing 'name' field (bug regression!)"
            assert isinstance(info["name"], str), f"Custom wordlist '{key}' has non-string 'name' field"
            assert len(info["name"]) > 0, f"Custom wordlist '{key}' has empty 'name' field"
    ...
    def test_get_wordlist_info_includes_name(self):
        ...
        for wordlist_key in all_wordlists[:5]:  # Test first 5 to keep test fast
            ...
            assert "name" in info, f"get_wordlist_info('{wordlist_key}') missing 'name' field"
            assert "key" in info, f"get_wordlist_info('{wordlist_key}') missing 'key' field"
            assert info["key"] == wordlist_key, f"get_wordlist_info('{wordlist_key}') returned wrong key"
    ...
    def test_wordlist_name_is_human_readable(self):
        ...
        for key, info in metadata["wordlists"].items():
            ...
            assert name != key, f"Wordlist '{key}' has name identical to key (should be human-readable)"

            # Name should not contain underscores (should use spaces)
            assert "_" not in name, f"Wordlist '{key}' name contains underscores: '{name}' (should use spaces)"
...
class TestWordlistAPIMetadata:
    ...
    def test_list_wordlists_returns_complete_metadata(self, app_client):
        ...
        for wl in data["wordlists"]:
            ...
            assert "key" in wl, "Wordlist missing 'key' in API response"
    ...
    def test_get_specific_wordlist_includes_name(self, app_client):
        ...
        assert "name" in data["metadata"], f"Wordlist '{test_wordlist}' metadata missing 'name' field in API response"
    ...
    def test_custom_wordlists_in_api_have_name(self, app_client):
        ...
        for wl in custom_wordlists:
            assert "name" in wl, f"Custom wordlist '{wl.get('key')}' missing 'name' in API response (bug regression!)"
            assert isinstance(wl["name"], str), f"Custom wordlist '{wl.get('key')}' has non-string 'name'"
            assert len(wl["name"]) > 0, f"Custom wordlist '{wl.get('key')}' has empty 'name'"
    ...
    def test_uploaded_wordlist_gets_name_field(self, app_client):
        ...
        assert "name" in data["metadata"], "Uploaded wordlist missing 'name' field in metadata"
        assert data["metadata"]["name"] == "Test Upload Wordlist", "Uploaded wordlist has wrong name"
    ...
    def test_wordlist_list_includes_display_names(self, app_client):
        ...
        for wl in wordlists[:10]:  # Test first 10
            ...
            assert name != key, f"Wordlist '{key}' name is identical to key (not human-readable)"
