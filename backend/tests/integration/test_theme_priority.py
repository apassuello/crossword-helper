...
class TestThemeWordPriorityOrdering:
    ...
    def test_theme_word_receives_50_bonus(self):
        ...
        from cli.src.fill.beam_search.selection.value_ordering import (
...
class TestThemeWordCLIIntegration:
    ...
    def test_cli_loads_theme_wordlist_file(self):
        ...
        try:
            ...
            assert "Loaded 5 theme words" in stdout or "5 theme words" in stdout, "CLI should report loading 5 theme words"
...
class TestThemeWordAPIIntegration:
    ...
    def test_fill_endpoint_accepts_theme_list_parameter(self, app_client):
        ...
        response = app_client.post("/api/fill", json=request_data, content_type="application/json")
        ...
        if response.status_code == 400:
            ...
            assert (
                "themeList" not in str(data.get("error", "")).lower()
    ...
    def test_theme_entries_flag_preserves_words(self):
        ...
        try:
            ...
            if json_result is None:
                pytest.fail(f"CLI did not produce JSON output (rc={result.returncode}); " f"stderr: {result.stderr[:500]}")

            filled_grid = json_result.get("grid")
            if filled_grid is None:
                pytest.fail(f"JSON output has no 'grid' key: {list(json_result.keys())}")

            # Check whether CAT is preserved at row 0, cols 0-2
            row0 = filled_grid[0]
            cat_preserved = row0[0] == "C" and row0[1] == "A" and row0[2] == "T"

            assert cat_preserved, f"--theme-entries did not preserve 'CAT' at row 0. " f"Got: {row0[:3]}. Full row: {row0}"
