...
class TestSSEMessageFormatCompliance:
    ...
    def test_sse_message_sequence_for_autofill(self, client, sse_parser):
        ...
        assert (
            len(messages) >= 2
        ), f"Expected multiple progress updates, got {len(messages)}"
        ...
        last_msg = messages[-1]
        assert (
            last_msg.get("status") in ["complete", "error"]
            or last_msg.get("progress") == 100
    ...
    def test_sse_message_contains_required_fields(self, client, sse_parser):
        ...
        for msg in messages:
            ...
            assert (
                "message" in msg or "progress" in msg
