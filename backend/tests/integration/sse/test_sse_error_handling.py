...
class TestSSEUserFriendlyMessages:
    ...
    def test_error_messages_contain_useful_info(self, client, sse_parser):
        ...
        if response.status_code == 400:
            ...
            assert "Traceback" not in message_text, "Error message should not include Python traceback"
            assert 'File "' not in message_text, "Error message should not include file paths from traceback"
