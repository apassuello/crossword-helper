...
def test_api_adaptive_beam_starts_successfully(client):
    ...
    assert response.status_code == 202, f"Expected 202 (task started), got {response.status_code}: {response.data}"
...
def test_adaptive_mode_adds_black_squares():
    ...
    try:
        ...
        if json_result is None:
            pytest.skip(f"CLI did not produce JSON output (rc={result.returncode}); " f"stderr: {result.stderr[:300]}")
